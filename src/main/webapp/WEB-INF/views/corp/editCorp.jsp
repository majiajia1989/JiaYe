...
<c:set var="corp" value='<%=request.getAttribute("corpCommand")%>' />
<c:set var="corpType" value='2' />
<c:set var="corpID" value="-1" />
<c:set var="corpProvince" value="" />
<c:set var="corpCity" value="" />
<c:set var="corpCounty" value="" />
<c:if test="${!empty corpCommand}">
	<c:set var="corpID" value="${corp.corp_id}" />
	<c:set var="corpType" value="${corp.type}" />
	<c:set var="corpProvince" value="${corp.province}" />
	<c:set var="corpCity" value="${corp.city}" />
	<c:set var="corpCounty" value="${corp.county}" />
</c:if>
<c:if test="${empty corpProvince}">
	<c:set var="corpProvince" value="河南省" />
	<c:set var="corpCity" value="郑州市" />
	<c:set var="corpCounty" value="中原区" />
</c:if>
<!DOCTYPE html>
<html>
<head>
...
<body>
		...
		<div class="col-md-12 ">
			<div class="portlet box iframe-box ">
				<div class="portlet-title col-md-12">
					<div class="caption">
						<c:if test="${corpID>0}">
							<i class="fa fa-edit"></i>&nbsp;修改企业</c:if>
						<c:if test="${corpID==-1}">
							<i class="fa fa-plus"></i>&nbsp;添加企业</c:if>
					</div>
					<div class="actions">
						<a id="data_table_new_btn" class="btn iframe-btn btn-return"
							href="${ctx}/corp/corp"><i class="fa fa-chevron-left"></i>&nbsp;返&nbsp;回
						</a>
					</div>
				</div>
				<div class="portlet-body form">
					...
					<f:form class="corp-form form-validate  form-horizontal"
						action="${ctx}/corp/corp" method="post" commandName="corpCommand">
						<input type="hidden" id="corp_id" name="corp_id" value="${corpID}" />
						<input type="hidden" id="corpProvince" name="corpProvince"
							value="${corpProvince}" />
						<input type="hidden" id="corpCity" name="corpCity"
							value="${corpCity}" />
						<input type="hidden" id="corpCounty" name="corpCounty"
							value="${corpCounty}" />
						<div class="form-body">
							<div class="row">
								...
								<div
									...
									<div class="col-md-8">
										<f:input class="form-control required" type="text"
											autocomplete="off" placeholder="单位名称" path="name" />
									</div>
								...
								<div class="form-group ">
									...
									<div class="col-md-8">
										<select class="form-control" id="type" name="type" path="type">
											<option <c:if test="${corpType eq 1}">selected="true"</c:if>
												value="1">系统用户</option>
											<option <c:if test="${corpType eq 2}">selected="true"</c:if>
												value="2">物业</option>
											<option <c:if test="${corpType eq 3}">selected="true"</c:if>
												value="3">商户</option>
										</select>
									</div>
									<label class="control-label"><span class="required">*</span></label>
								...
								<div
									...
									<div class="col-md-8">
										<f:textarea type="text" class="form-control required" rows="6"
	...
	<![endif]-->
		...
		<script type="text/javascript"
			src="<c:url value='/assets/scripts/pages/editCorp.js'/>"></script>
