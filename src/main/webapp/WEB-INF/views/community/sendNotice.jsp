		...
		<div class="col-md-12 ">
			<div class="portlet box iframe-box ">
				<div class="portlet-title">
					<div class="caption">
						<i class="fa fa-reorder"></i>发送通知
					</div>
					<div class="actions">
						<a id="send-btn" class="btn iframe-btn" href="#"> <i
							class="fa fa-check"></i>&nbsp;发&nbsp;送
						</a> <a id="data_table_new_btn" class="btn iframe-btn btn-return"
							href="${ctx}/community/notices"><i
							class="fa fa-chevron-left
"></i>&nbsp;返&nbsp;回 </a>
					</div>
				</div>
				<div class="portlet-body">
					<div class="form-group sendNotice">
						<div class="col-md-12">
							<div class="col-md-10">
								<label class="control-label pull-left">楼号：</label>
								<div class="pull-left">
									<select class="multiselect" id="house-select"
										multiple="multiple">
										<option value="-1">全部</option>
										<c:forEach items="${houses}" var="house">
											<option value="${house.first}">${house.second}</option>
										</c:forEach>
									</select>
								</div>
								<label class="pull-left control-label"
									style="margin-left: 20px;">单元号：</label>
								<div class="pull-left">
									<select class="multiselect" id="unit-select"
										multiple="multiple">
										<option value="-1">全部</option>
										<c:forEach items="${units}" var="unit">
											<option value="${unit.first}">${unit.second}</option>
										</c:forEach>
									</select>
								</div>
								<label class="pull-left control-label"
									style="margin-left: 20px;">楼层：</label>
								<div class="pull-left">
									<select class="multiselect" id="floor-select"
										multiple="multiple">
										<option value="-1">全部</option>
										<c:forEach items="${floors}" var="floor">
											<option value="${floor.first}">${floor.second}</option>
										</c:forEach>
									</select>
								</div>
								<label class="pull-left control-label"
									style="margin-left: 20px;">是否有车：</label>
								<div class="pull-left">
									<select class="border-grey textwidth" id="hascar-select">
					...
					<table class="crl table table-striped table-bordered table-hover"
						id="data_table">
						<thead>
							<tr role="row">
								<th />
	...
	<![endif]-->
		...
		<script type="text/javascript"
			src="<c:url value='/assets/plugins/bootstrap-multiselect/js/bootstrap-multiselect.js'/>"></script>
		...
		<script type="text/javascript"
			src="<c:url value='/assets/scripts/pages/sendNotice.js'/>"></script>
		<!-- END PAGE LEVEL SCRIPTS -->
		<script>
			$(function() {				
				SendNotice.init("<c:url value='/' />", ${noticeID});
