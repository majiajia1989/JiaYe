var WaterTemplate = function() {
	var resolveRootUrl = "";
	var initList = function() {
		var url = resolveRootUrl + 'consumeTemplate/readConsumeTemplate?community=' + $("#community").val() + '&consumeType=' + $("#consumeType").val();
		$("#dtList").dataTable(
				{
					"bServerSide" : false,
					"sServerMethod" : "GET",
							...
							},
							{
								...
								"fnRender" : function(jsonData) {
									...
									var editInfo = '<button class="btn btn-xs iframe-btn btn-edit" style=" cursor:hand;" onclick="'
											+ "javascript:WaterTemplate.editInfo(\'"
											+ jsonData.aData.id
											+ "\',\'"
									...
									var deleteInfo = '<button class="btn btn-xs iframe-btn btn-delete" style=" cursor:hand;" onclick="'
											+ "javascript:WaterTemplate.deleteInfo(\'"
											+ jsonData.aData.id
											+ "\');" + '" >删除</button>';
									return '<div width="100px">'+ editInfo + '&nbsp'
											+ deleteInfo+'</div>';
			...
			bootbox.dialog({
				...
				buttons : {
					success : {
						...
						callback : function() {
							$.post(resolveRootUrl + 'consumeTemplate/deleteConsumeTemplate',
