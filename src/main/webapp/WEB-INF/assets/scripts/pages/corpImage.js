var CorpImage = function() {
	var initTable = function(ctxUrl) {
		if (!jQuery().dataTable) {
			return;
		}

		var dataUrl = ctxUrl + 'corp/readCorpImage';
		$('#data_table')
				.dataTable(
						{
							"bServerSide" : true,
							...
							"aoColumns" : [
									...
									},									
									{
										...
										"mRender" : function(data, type, full) {
											if (type == "display") {
												var removeUrl = "javascript:CorpImage.removeByID('"
														+ ctxUrl
														+ "','"
														+ data
														+ "');";
												var removeA = '<a href="#" class="btn iframe-btn btn-delete  btn-xs" onclick='
													+ removeUrl + '>删除</a>';												
												var setDefaultUrl="javascript:CorpImage.setDefaultImageByID('"+ctxUrl+"','"+data+"');";
												var cancleDefaultUrl="javascript:CorpImage.cancleDefaultImageByID('"+ctxUrl+"','"+data+"');";
	...
	var deleteMsg = function(ctxUrl, id) {
		var url = ctxUrl + "corp/removeImageByID/" + id;
	...
	var setDefaultImage=function(ctxUrl,id){
		var url=ctxUrl+"corp/setDefaultImage/"+id;
		$.post(url,'',function(json){
			if(json.success){
				$('#data_table').dataTable().fnDestroy();
				initTable(ctxUrl);
	...
	var cancleDefaultImage=function(ctxUrl,id){
		var url=ctxUrl+"corp/cancleDefaultImage/"+id;
		$.post(url,'',function(json){
			if(json.success){
				$('#data_table').dataTable().fnDestroy();
				initTable(ctxUrl);
